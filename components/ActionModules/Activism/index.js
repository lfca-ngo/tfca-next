...
const ActivismFlow = (props) => {
  const { goTo, index, setStore, store } = useFlow({
    initial: 'intro',
    name: props.name,
  })

  const organizations = props.module?.data['organizations']

  const availableOrganizationTypes = useMemo(
    () => getFilterOptions(organizations, 'tagsCollection'),
    [organizations]
  )
        ...
        {[...steps.keys()].map((key) => {
          ...
          return (
            <TabPane key={key} tab={`${props.name}`}>
              <Page
                availableOrganizationTypes={availableOrganizationTypes}
